...
export default class GrokController {
  ...
  async generateContent({ request, response, auth }: HttpContext) {
    try {
      ...
      const {
        ...
        tone = 'casual',
        maxLength = 280,
      } = request.only(['prompt', 'tone', 'maxLength'])
      ...
      console.log(
        `🤖 Génération de contenu Grok pour ${user.username}: "${prompt}" (tone: ${tone})`
      )
  ...
  async suggestHashtags({ request, response, auth }: HttpContext) {
    try {
      ...
      console.log(
        `🏷️ Suggestions hashtags Grok pour ${user.username}: "${content.substring(0, 50)}..."`
      )
  ...
  async analyzeTweet({ request, response, auth }: HttpContext) {
    try {
      ...
      console.log(
        `📊 Analyse tweet Grok pour ${user.username}: "${tweetContent.substring(0, 50)}..."`
      )
      ...
      if (result.success) {
        console.log(
          `✅ Analyse terminée: sentiment ${result.data.sentiment}, score ${result.data.engagement_score}`
        )
        return response.ok(result)
      } else {
        return response.internalServerError(result)
  ...
  async optimizeTweet({ request, response, auth }: HttpContext) {
    try {
      ...
      console.log(
        `✅ Optimisation terminée avec ${result.data.improved_suggestions.length} suggestions`
      )
      return response.ok(result)
    } catch (error) {
